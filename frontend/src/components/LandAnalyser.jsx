  ...
  <div className="min-h-screen pt-24 flex items-center justify-center bg-[#fafaf9] px-6">
    ...
    <div className="max-w-lg w-full">
      ...
      <div className="space-y-4">
        <button onClick={onGpsDetect} disabled={detecting}
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1">
              <h3 className="font-black text-lg text-[#0c0a09] mb-1">Use My Location</h3>
              <p className="text-stone-500 text-sm font-medium">Auto-detect via GPS for the most accurate analysis.</p>
              {detecting && <div className="flex items-center gap-2 mt-3 text-[#84cc16] font-bold text-sm"><Loader2 className="w-4 h-4 animate-spin" />Detecting location...</div>}
            </div>
        ...
        <div className="relative"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-stone-200"></div></div><div className="relative flex justify-center"><span className="bg-[#fafaf9] px-4 text-xs font-bold text-stone-400 uppercase">or</span></div></div>
        <button onClick={onCitySearch}
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1">
              <h3 className="font-black text-lg text-[#0c0a09] mb-1">Search City</h3>
              <p className="text-stone-500 text-sm font-medium">Type your city or town name from Karnataka.</p>
            </div>
            <div className="w-6 h-6 rounded-full border-2 border-stone-300 flex items-center justify-center flex-shrink-0 mt-1 group-hover:border-[#84cc16] transition-colors">
              <div className="w-3 h-3 rounded-full group-hover:bg-[#84cc16] transition-colors"></div>
            </div>
  ...
  return (
    <div className="min-h-screen pt-24 bg-[#fafaf9] px-6 flex items-start justify-center">
      ...
      <div className="max-w-lg w-full mt-12">
        <button onClick={onBack} className="text-stone-400 hover:text-stone-600 font-bold text-sm mb-6 flex items-center gap-1 transition-colors">← Back</button>
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#84cc16] mb-2">Step 2 of 2</p>
        <h2 className="font-serif text-4xl font-black text-[#0c0a09] mb-2">Search <span className="italic text-[#84cc16]">City</span></h2>
        <p className="text-stone-500 font-medium mb-6">Type a Karnataka city or town name.</p>
        <div className="relative">
          ...
          <input value={query} onChange={e => { setQuery(e.target.value); setSelected(null); }}
            placeholder="e.g. Bengaluru, Mysuru, Chikkaballapura..."
        ...
        {selected && (
          <button onClick={() => onSelect(selected.city, selected.lat, selected.lon)} disabled={loading}
            ...
            {loading ? 'Analysing...' : `Analyse ${selected.city}`}
          </button>
        )}
      </div>
    </div>
    ...
    <div className="pt-24 min-h-screen bg-[#fafaf9]">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20 space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#84cc16] mb-2">VaniAI Geospatial Intelligence</p>
            <h1 className="font-serif text-5xl font-black text-[#0c0a09]">Land <span className="italic text-[#84cc16]">Analysis</span></h1>
          </div>
          <button onClick={() => { setStep('choose'); setResult(null); setError(null); }}
            className="text-sm font-bold text-stone-500 hover:text-[#84cc16] flex items-center gap-1 transition-colors"><MapPin size={14} /> New Analysis</button>
        </div>
        ...
        <div className="bg-gradient-to-r from-[#84cc16]/10 to-transparent rounded-[2rem] p-6 border border-[#84cc16]/20 flex items-center justify-between">
          <div>
            <p className="text-[8px] font-black uppercase text-[#84cc16]">📍 Live Location</p>
            <p className="font-black text-xl text-[#0c0a09]">{r.location?.city || '—'}</p>
            <p className="text-xs text-stone-500">{r.location?.lat?.toFixed(4)}, {r.location?.lon?.toFixed(4)} · Elev: {r.topography?.elevation_m ?? '—'}m</p>
          </div>
            ...
            <>
              ...
              {s.deficiencies?.length > 0 && (
                <div className="bg-red-50 rounded-xl p-3 border border-red-200">
                  <p className="text-[8px] font-black uppercase text-red-600 mb-2">⚠ Soil Deficiencies</p>
                  {s.deficiencies.map((d, i) => <p key={i} className="text-xs text-stone-700 mb-1">• {d.element}: {d.status} — {d.advice}</p>)}
                </div>
              )}
            </>
          ) : <p className="text-stone-400 text-sm italic">Soil data unavailable for this location.</p>}
        </div>
        ...
        {recs && (
          <div className="grid md:grid-cols-2 gap-4">
            {recs.soil_amendments?.length > 0 && (
              <div className="bg-white rounded-[2rem] p-6 border-2 border-stone-200 shadow-lg">
                <h3 className="text-[8px] font-black uppercase text-emerald-600 mb-3">🧪 Soil Amendments</h3>
                {recs.soil_amendments.map((r, i) => <p key={i} className="text-xs text-stone-700 mb-1">→ {r}</p>)}
              </div>
            )}
            {recs.irrigation_advice?.length > 0 && (
              <div className="bg-white rounded-[2rem] p-6 border-2 border-stone-200 shadow-lg">
                <h3 className="text-[8px] font-black uppercase text-blue-600 mb-3">💧 Irrigation Advice</h3>
                {recs.irrigation_advice.map((r, i) => <p key={i} className="text-xs text-stone-700 mb-1">→ {r}</p>)}
              </div>
            )}
            {recs.crop_rotation?.length > 0 && (
              <div className="bg-white rounded-[2rem] p-6 border-2 border-stone-200 shadow-lg">
                <h3 className="text-[8px] font-black uppercase text-[#84cc16] mb-3">🔄 Crop Rotation</h3>
                {recs.crop_rotation.map((r, i) => <p key={i} className="text-xs text-stone-700 mb-1">→ {r}</p>)}
              </div>
            )}
            {recs.conservation?.length > 0 && (
              <div className="bg-white rounded-[2rem] p-6 border-2 border-stone-200 shadow-lg">
                <h3 className="text-[8px] font-black uppercase text-amber-600 mb-3">🌍 Conservation</h3>
